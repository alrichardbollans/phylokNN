...
data_path = os.path.join(repo_path, 'PhyloNN', 'analysis', 'data', 'gentianales')
continuous_input_path = os.path.join(data_path, 'continuous')
binary_input_path = os.path.join(data_path, 'binary')

continuous_output_path = os.path.join('predictions', 'continuous')
binary_output_path = os.path.join('predictions', 'binary')


def predict(continuous: bool):
    if continuous:
        clf = False
        out_dir = os.path.join(continuous_output_path)
        val_scorer = make_scorer(nan_safe_metric_wrapper(mean_absolute_error), greater_is_better=False)
        input_path = os.path.join(continuous_input_path, 'outputs', 'group_data')
        missing_values_csv = os.path.join(input_path, 'continuous_gentianales_mcar_sample.csv')
        distance_csv = os.path.join(repo_path, 'gentianales_trees','WCVP_12','Uphy',  'outputs', 'Genus', 'genus_distances.csv')

    else:
        clf = True
        out_dir = os.path.join(binary_output_path)
        val_scorer = make_scorer(nan_safe_metric_wrapper(brier_score_loss), greater_is_better=False)
        input_path = os.path.join(binary_input_path)
        missing_values_csv = os.path.join(input_path, 'binary_gentianales_mcar_sample.csv')
