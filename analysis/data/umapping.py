...
def supervised(dir_path: str, reduction_fraction=reduction_factor):
    # Note this is only available for classification problems.
    #  To make thhis semisupervised, just add -1 to unlabelled cases.
    distances = pd.read_csv(os.path.join(dir_path, 'tree_distances.csv'), index_col=0)
    scaled_penguin_data = StandardScaler().fit_transform(distances)
    fitter = umap.UMAP(n_components=int(len(distances.columns) * reduction_fraction))

    ground_truth = pd.read_csv(os.path.join(dir_path, 'ground_truth.csv'), index_col=0)
    target_name = ground_truth.columns.to_list()[0]

    fitter.fit(scaled_penguin_data, y=ground_truth[target_name].values)
